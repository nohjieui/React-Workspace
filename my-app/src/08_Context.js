/*
    ...
    단, 해당 데이터를 자식컴포넌트가 아니라 후손 컴포넌트에서만 필요로하는 데이터라면 자식컴포넌트에서는
    불필요하게 코드를 작성해줘야 하는데, 이때 Context를 사용하면
    데이터 공급자(Provider)와 소비자(Consumer)를 정의하여 데이터가 필요한 컴포넌트만 사용할 수 있게 구현할 수 있다.

    Context를 사용하면 하위컴포넌트가 여러개인 구조에서 유용하게 사용할 수 있음
...
class ContextApi extends React.Component{
    ...
    render(){
        ...
        return(
            // 자식태그(Children)를 Provider로 감싸고 전달할 테이터를 value값으로 할당
